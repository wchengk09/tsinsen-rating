...
function biShiftLeft(b, h) {
    ...
    for (var e = a.digits.length - 1, f = e - 1; e > 0; --e,
    --f) {
...
function biShiftRight(b, h) {
    ...
    for (var d = 0, e = d + 1; d < a.digits.length - 1; ++d,
    ++e) {
...
function biDivideModulo(g, f) {
    ...
    for (var z = u; z > k; --z) {console.log("z",z);
...
var username="guwanghaohao";
var password="123456gwhh";

const http = require('http');

var req = http.request('http://sy.hhwdd.com/user.SecureLogin.dt', {method: 'POST',headers: { 'Content-Type': 'application/json' }}, (res) => {
    let data = '';
    ...
    res.on('end', () => { 
        ...
        if(data.ret == "OK"){
            console.log(data);
            var nextreq = http.request('http://sy.hhwdd.com/user.SecureLogin.dt', {method: 'POST',headers: { 'Content-Type': 'application/json' }}, (res) => {
                let data='';
                res.on('data', (chunk) => { data += chunk; });
                res.on('end', () => { 
                    data=JSON.parse(data);
            ...
            var c = biFromHex(data.mod)
            console.log(c);
            var d = biFromHex(data.pub);
            console.log(d);
            var g = biFromHex(hexOfString("HWD," + data.ti + "," + password));
            console.log(g);
            var e = biPowMod(g, d, c);
            console.log(e);
            nextreq.write(JSON.stringify({cmd: 's2',login: username,hash:biToHex(e)}));
            nextreq.end();
        }
    });
});
req.write(JSON.stringify({ cmd: 's1' }));
req.end();